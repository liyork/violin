...
/**
 * <p> Description:动态代理，不用修改原类逻辑，动态添加额外功能。
 * 可以在原始类和接口还未知的时候，就确定代理类的代理行为，解耦。
...
//		}
    ...
    public void testBase() throws IllegalAccessException, InvocationTargetException {
        ...
        UserDao aa = (UserDao) getProxy(userDaoImpl);
        //final class $Proxy0 extends Proxy implements UserDao
        //生成的proxy类就是UserDaoImpl的代理，
        // 其中的test方法调动this.h.invoke(this, m3, null);
        //m3 = Class.forName("com.wolf.test.base.proxy.UserDao").getMethod("test", new Class[0]);
    ...
    public void doubleWrap() throws InvocationTargetException, IllegalAccessException {
        final UserDaoImpl bb = new UserDaoImpl();

        UserDao aa = (UserDao) getProxy(bb);
        aa.test();

        UserDao cc = (UserDao) getProxy(aa);
        cc.test();
    }

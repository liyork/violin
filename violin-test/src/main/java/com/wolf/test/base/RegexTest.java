...
public class RegexTest {
    ...
    public void test1() {
        ...
        String s1 = m1.replaceAll("");//将匹配的都替换成""，得到的结果是没匹配到的
...
//		String text2="<div>xxxx</div>ababab<div>11111</div>";//好像>自身带有费贪婪，上面改成q就不行了。。
    ...
    public void test4() {
        ...
        if (m_script.find()) {
            String group = m_script.group(0);// 整段
            System.out.println(group);
            String group1 = m_script.group(1);// 捕获组
    ...
    public void test6() {
        ...
        if (m_script.find()) {
            String group = m_script.group(0);// 整体
            System.out.println(group);

            String group1 = m_script.group(1);// 第一个捕获组
            System.out.println(group1);

            //此处只为展示，正常正则不会用这个()
            String group2 = m_script.group(2);// 第二个捕获组
    ...
    // 正则中的\有特殊含义，要是想表达\字符本意，那么需要用\针对\进行转义
    @Test
    public void test10() {
        ...
        // 输出时，先计算，s2aaasdsad\rbb，然后定位到开始，再输出bb
    ...
    public void testNonCapture() {
        ...
        //(?:pattern)非获取匹配，匹配pattern但不获取匹配结果，不进行存储，所以以后也不能使用
        Pattern p1 = Pattern.compile("industr(?:y|ies)");
        Matcher m1 = p1.matcher(string1);
        if (m1.find()) {
            System.out.println("m1==>" + m1.group());// 等于group(0)
            //报错，因为(?:pattern)不捕获
    ...
    public void testRemoveChinese() {
        //仅中文替换
        System.out.println("中ab哈c过".replaceAll("[\\u4e00-\\u9fa5]", ""));
        // ascii码替换
        System.out.println("中ab哈c过".replaceAll("[\\x00-\\xff]", ""));
        //非ascii码替换(中文，全角啥的)
        System.out.println("中ab哈c过".replaceAll("[^\\x00-\\xff]", ""));

        // 这里的ｃ是全角
        System.out.println("ｃ中ab哈c过".replaceAll("[^\\x00-\\xff]", ""));
        // 仅替换中文
        System.out.println("ｃ中ab哈c过".replaceAll("[\\u4e00-\\u9fa5]", ""));
    }
    ...
    //matches匹配全部时才能true，find匹配到部分就能返回true(当前前提是没有^$，否则^$就要进行全部匹配了)
    @Test

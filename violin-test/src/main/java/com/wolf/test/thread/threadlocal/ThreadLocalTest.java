 ...
 * 内部使用原理：
 * threadLocal只是用来操作每个线程的工具，每个线程内部都有个map用来存放每个threadlocal对象的
 * threadLocal的get方法，先从当前thread获取其内部的map，然后操作，一个引用。
 * map使用开方查找而非链表查找
 *
 * 为什么在threadLocal中维护一个map，key：threadid，value：值？
 * 那样会出现多线程问题而且map中关联所有线程，不易垃圾回收，连接过多。
 * <p/>
 * Date: 2015/9/28
 * Time: 19:39
 *
...
//		}
	...
	private static void testThreadLocalIsRight(final Person person) throws InterruptedException {
	...
	//测试多个tl对于一个线程，内部是如何构造的。使用取模table-1，不在则新建，如果有冲突则nextIndex。
	@Test

...
/*动态创建代理类*/
        ...
        try {
/*获得DProxy类作为代理类的父类*/
            CtClass superclass = pool.get("com.wolf.test.javassist.DProxy");
...
/*创建构造方法以便注入拦截器*/
            CtConstructor cc = new CtConstructor(new CtClass[]{pool.get("com.wolf.test.javassist.Interceptor")}, proxy);
            cc.setBody("{super($1);}");
            proxy.addConstructor(cc);
            proxy.writeFile("D:\\myproxy");

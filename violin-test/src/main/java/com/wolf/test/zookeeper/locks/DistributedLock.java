...
public class DistributedLock {
	...
	public String createTempNode(String lockNodePath)
		...
		//防止客户端由于ConnectionLossException导致重新连接重新创建一个节点，导致以前的节点删除不掉，导致死循环(客户端会话结束前)。。。
		List<String> children = zk.getChildren(lockNodePath, false);
		for (String child : children) {
			String tempSessionId = child.split("-")[1];
			if (sessionId == Long.parseLong(tempSessionId)) {
				return child;
			}
		}

		String path = lockNodePath + "/lock-" + sessionId + "-";
		...
		for (int i = 0; i < 10; i++) {
			new Thread(new Runnable() {
				...
				public void run() {
					...
					try {
						//TODO 分布式时如何阻塞当前进程？
						isLock = distributedLock.lock(lockNodePath);
						if (!isLock) {
							//TODO 如果未获取到锁，需要当前机器进程等待
						}
					} catch (Exception e) {
						e.printStackTrace();
					} finally {
		...
		for (int i = 0; i < 5; i++) {
			new Thread(new Runnable() {
				...
				public void run() {
					DistributedLock distributedLock = new DistributedLock();
					try {
						distributedLock.lock(lockNodePath);
					} catch (Exception e) {
						e.printStackTrace();
					} finally {
//						distributedLock.unLock();

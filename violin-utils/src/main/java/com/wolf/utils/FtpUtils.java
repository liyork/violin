...
public class FtpUtils {
	...
	public static final String REMOTE_ADDRESS = CONFIG.getProperty("ftp.xx.host");
	public static final String REMOTE_PATH = CONFIG.getProperty("ftp.xx.remotePath");
	public static final String REMOTE_HOME = CONFIG.getProperty("ftp.xx.remoteHome");

	public static final String REMOTE_PORT = CONFIG.getProperty("ftp.xx.port");
	private static final String PASSWORD = CONFIG.getProperty("ftp.xx.password");
	private static final String USER_NAME = CONFIG.getProperty("ftp.xx.name");
	...
	public static Properties getConfigProperties(){
		if(CONFIG == null){
			...
			try {
				p.load(FtpUtils.class.getResourceAsStream("/ftp.properties"));
				return p ;
			} catch (IOException e) {
				throw new RuntimeException("ftp配置文件加载失败！");
			}
...
//				boolean flag =
	...
	/**
	 * 根据传入的路径建立输出流
	 * @Version1.0 2014-7-9 上午11:55:14 by 张延伟（yw.zhang02@xx.com）创建
	 * @param path 文件保存路径
	 * @param fileName 文件名称
	 * @return
	 */
	...
	/**
	 * 判断文件是否存在
	 * @Version1.0 2014-7-10 上午11:34:30 by 张延伟（yw.zhang02@xx.com）创建
	 * @param path 文件路径
	 * @param fileName 文件名称
	...
	private static Properties getFtpPathConfig(){
		synchronized (FtpUtils.class) {
			if (ftpPathConfig == null) {
				...
				InputStream in = null;
				try {
					in = FtpUtils.class.getResourceAsStream("/ftpUploadPathConfig.properties");
					p.load(in);
				} catch (IOException e) {

					throw new RuntimeException("ftp上传路径失败！",e);
				}finally{
					if(in != null){
						try {
							in.close();

...
public abstract class MyRateLimiter {
    ...
    static MyRateLimiter create(double permitsPerSecond, SleepingStopwatch stopwatch) {
        MyRateLimiter rateLimiter = new MySmoothRateLimiter.MySmoothBursty(stopwatch, 1.0 /* maxBurstSeconds */);
    ...
    @VisibleForTesting
    static MyRateLimiter create(
            double permitsPerSecond,
            long warmupPeriod,
            TimeUnit unit,
            double coldFactor,
            SleepingStopwatch stopwatch) {
        MyRateLimiter rateLimiter = new MySmoothRateLimiter.MySmoothWarmingUp(stopwatch, warmupPeriod, unit, coldFactor);
        rateLimiter.setRate(permitsPerSecond);
        return rateLimiter;
    }
    ...
    /**
     * Acquires permits from this {@link MyRateLimiter} if it can be acquired immediately without delay.
     *

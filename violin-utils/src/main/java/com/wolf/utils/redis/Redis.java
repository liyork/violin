...
public class Redis extends PoolRedis {
    ...
    @Override
    public List<Map<byte[], byte[]>> hGetAllKeys(byte[] ...keys) {
    	
    	for(byte[] key : keys){
    		client.hgetAll(key);
    	}
    	//设置pipeline 监控
    	String command = ExecuteContext.COMMAND_THREAD_LOCAL.get() ;
    	if(command != null){
    		ExecuteContext.COMMAND_THREAD_LOCAL.set("pipeline_"+command);
    	}
    	
    	client.flush();
    	
        ArrayList<Map<byte[], byte[]>> resultList = new ArrayList<Map<byte[],byte[]>>();
    	for(int i =0;i<keys.length ; i++){
    		final List<byte[]> flatHash = client.getBinaryMultiBulkReplyWithoutFlush();
            final Map<byte[], byte[]> hash = new TedisByteHashMap();
            final Iterator<byte[]> iterator = flatHash.iterator();
            while (iterator.hasNext()) {
                hash.put(iterator.next(), iterator.next());

...
public class FileUtils {
	...
	public static void readFilePerBytes(String fileName) {
		...
		try {
			...
			while ((byteread = in.read(tempbytes)) != -1) {
				//使用gbk的原因是，a.txt文件就是gbk
				System.out.println(new String(tempbytes, 0, byteread, "gbk"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	 ...
	 */
	public static void readFilePerLine(String fileName) {
		BufferedReader reader = null;
		try {
			//乱码，由于intellij设定charset为utf-8，而a.txt是gbk
	...
	public static void readFileRandom(String fileName) {
		...
		try {
			...
			while ((readByteLength = randomFile.read(bytes)) != -1) {
				System.out.print(new String(bytes, 0, readByteLength, "gbk"));
			}
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
...
//		appendContent2File(fileName,"四五","gbk");
//		writeContent2File(fileName, "123哈123哈123", true, "gbk");

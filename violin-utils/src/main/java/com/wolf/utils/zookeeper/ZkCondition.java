...
final class ZkCondition extends ZkPrimitive implements Condition {
    ...
    public void awaitUninterruptibly() {
        ...
        try {
            ...
            String conditionName = zooKeeper.create(baseNode + "/" + CONDITION_PREFIX + CONDITION_DELIMITER,
    ...
    public void signal() {
        ...
        ZooKeeper zooKeeper = zkSessionManager.getZooKeeper();
        try {
            List<String> conditionsToSignal = ZkInternalUtils.filterByPrefix(zooKeeper.getChildren(baseNode, false), CONDITION_PREFIX);
            if(conditionsToSignal.size()<=0){
            	return; //no parties to signal
            }

            //delete the lowest numbered waiting party
            ZkInternalUtils.sortBySequence(conditionsToSignal, CONDITION_DELIMITER);

...
public final class ZkSessionPoller {
    private static final Logger CLASS_LOGGER = LoggerFactory.getLogger(ZkSessionPoller.class);
    ...
    private final class SessionPoller implements Runnable{
        ...
        public void run() {
            ...
            if(CLASS_LOGGER.isTraceEnabled()){
            	CLASS_LOGGER.trace("current state of ZooKeeper object: "+ zk.getState());
            }
            try{
                zk.exists("/",false);
                ...
                if(e.code()== KeeperException.Code.SESSIONEXPIRED){
                	...
                	CLASS_LOGGER.debug("Received a ConnectionLoss Exception, determining if our session has expired");
    ...
    private void expire() {
        //session expired!
    	CLASS_LOGGER.info("Session has expired, notifying listenerand shutting down poller");
...
//        rootLogger.addAppender(new ConsoleAppender(new SimpleLayout()));
        ...
        final ZooKeeper zk = new ZooKeeper("127.0.0.1:2181",2000,new Watcher() {
            @Override
        ...
        ZkSessionPoller poller = new ZkSessionPoller(zk,TWO_HUNDRED,new ConnectionListenerSkeleton() {
